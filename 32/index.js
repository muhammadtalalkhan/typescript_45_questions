...
var current_users = ["john", "eric", "sam", "daniel", "david"];
var new_users = ["Mike", "Trevor", "Carl", "Sam", "John"];
var lower_current_users = current_users.map(function (user) { return user.toLowerCase(); });
for (var i = 0; i < new_users.length; i++) {
    var lower_new_user = new_users[i].toLowerCase();
    if (lower_current_users.includes(lower_new_user)) {
        console.log("".concat(new_users[i], " is already used."));
    }
    else {
